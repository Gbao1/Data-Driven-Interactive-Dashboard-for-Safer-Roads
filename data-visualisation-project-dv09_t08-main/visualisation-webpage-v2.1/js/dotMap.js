...
class DotMap {
  constructor() {
    ...
    // Australian jurisdiction coordinates (more accurate lat/lon)
    this.jurisdictionCoords = {
      NSW: { lat: -32.0, lon: 147.0 },
      VIC: { lat: -37.0, lon: 145.0 },
      QLD: { lat: -23.0, lon: 143.0 },
  ...
  updateSize() {
    ...
    // Update if we have valid data - removed the isActiveChart check
  ...
  drawMap() {
    ...
    if (!this.g || !this.pathGenerator) {
      console.error("SVG group or pathGenerator not initialised.")
      return
    }
    ...
    this.g
      ...
      .style("fill", "#f8f9fa") // Slightly darker background for better contrast
      .style("stroke", "#dee2e6")
  ...
  update() {
    ...
    const selectedYear = window.chartNavigation
      ? window.chartNavigation.getSelectedValues("jurisdiction-year-filter")
      : []
    ...
    // Clear previous dots
    this.g.selectAll(".jurisdiction-dot").remove()
    this.g.selectAll(".jurisdiction-label").remove()
    ...
    // Create proper scaling for dot sizes
    const maxFines = d3.max(mapData, (d) => d.fines)
    const minFines = d3.min(mapData, (d) => d.fines)
    const radiusScale = d3.scaleSqrt().domain([minFines, maxFines]).range([10, 45]) // Increased minimum size

    // Improved color scale with better contrast and range
    const colorScale = d3
      .scaleSequential(d3.interpolateViridis) // Colour diverging scale for better contrast
      .domain([minFines, maxFines])

    // Draw dots
    const dots = this.g
      .selectAll(".jurisdiction-dot")
      .data(mapData)
      .enter()
      .append("circle")
      .attr("class", "jurisdiction-dot")
      .attr("cx", (d) => {
      ...
      .style("fill", (d) => colorScale(d.fines))
      .style("stroke", "#ffffff")
      .style("stroke-width", 2.5) // Thicker white border for better visibility
      .style("opacity", 0.85) // Slightly more opaque
    ...
    this.g
      ...
      .attr("y", (d) => {
        const coords = this.jurisdictionCoords[d.jurisdiction]
        const projectedY = coords && this.projection ? this.projection([coords.lon, coords.lat])[1] : 0
        return projectedY + radiusScale(d.fines) + 15
      })
      .attr("text-anchor", "middle")
    ...
    dots
      .on("mouseover", (event, d) => {
        d3.select(event.target)
          .transition()
          .duration(200)
          .attr("r", radiusScale(d.fines) * 1.1)
          .style("opacity", 1)

        window.tooltip.transition().duration(200).style("opacity", 0.9)
        window.tooltip
          .html(`Jurisdiction: ${d.jurisdiction}<br/>Total Fines: $${d.fines.toLocaleString()}`)
          .style("left", event.pageX + 10 + "px")
          .style("top", event.pageY - 28 + "px")
      })
      .on("mouseout", (event, d) => {
        d3.select(event.target).transition().duration(200).attr("r", radiusScale(d.fines)).style("opacity", 0.85)

        window.tooltip.transition().duration(500).style("opacity", 0)
      })

    // Add legend
    this.addLegend(mapData, radiusScale, colorScale)
