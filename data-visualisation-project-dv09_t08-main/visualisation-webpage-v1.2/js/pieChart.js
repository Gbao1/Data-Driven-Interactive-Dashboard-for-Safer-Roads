...
class PieChart {
  ...
  updateSize() {
    ...
    this.radius = Math.max(60, Math.min(availableWidth * 0.35, availableHeight * 0.35))
  ...
  update() {
    ...
    const selectedYears = window.chartNavigation.getSelectedValues("pie-year-filter")
    const selectedAgeGroups = window.chartNavigation.getSelectedValues("pie-age-group-filter")

    let filteredData = window.dataManager.getAgeGroupData()
    ...
    if (selectedAgeGroups.length > 0) {
      filteredData = filteredData.filter((d) => selectedAgeGroups.includes(d.ageGroup))
    }
    ...
    this.g
      ...
      .style("fill", "#666")
      .text(`Total: $${totalFines.toLocaleString()}`)
  ...
  addLegend(pieData) {
    ...
    // Calculate safe legend position
    const legendX = Math.max(this.width + this.margin.left - 110, this.width * 0.7)
    const legendY = 50
    ...
    items
      ...
      .text((d) => `$${(d.fines / 1000).toFixed(0)}k`)
...
// Initialize pie chart when data is loaded
